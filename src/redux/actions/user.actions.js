...
const addFriend = (user_id) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: { ...res.data.data, targetId: user_id },
    });
  } catch (error) {
...
const removeFriend = (user_id) => async (dispatch) => {
  dispatch({ type: types.REMOVE_FRIEND_REQUEST, payload: null });
  try {
    const res = await api.delete(`/friends/${user_id}`);
    dispatch({
...
const declineRequest = (user_id) => async (dispatch) => {
  dispatch({ type: types.DECLINE_REQUEST_REQUEST, payload: null });
  try {
    const res = await api.delete(`/friends/manage/${user_id}`);
    dispatch({
...
const acceptRequest = (user_id) => async (dispatch) => {
  dispatch({ type: types.ACCEPT_REQUEST_REQUEST, payload: null });
  try {
    const res = await api.post(`/friends/manage/${user_id}`);
    dispatch({
...
const cancelRequest = (user_id) => async (dispatch) => {
  dispatch({ type: types.CANCEL_REQUEST_REQUEST, payload: null });
  try {
    const res = await api.delete(`/friends/add/${user_id}`);
    dispatch({
