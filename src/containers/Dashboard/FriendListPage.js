...
import { actionTypes, friendListTabNames } from "../../config/constants";
...
const FriendListPage = () => {
  ...
  useEffect(() => {
    switch (tabKey) {
      case friendListTabNames.FRIENDS:
        dispatch(userActions.getFriendsRequest(pageNum, 10, query, sortBy));
        break;
  ...
  if (users) {
    console.log("Dashboard get all users: ", users);
  }
  ...
  const generateActions = (user) => {
    ...
    if (tabKey === friendListTabNames.ALL_USERS) {
      if (user && user.friendship && user.friendship.status === "accepted") {
        return (
